...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `${SITE_URL}/platforms`,
        'ko': `${SITE_URL}/ko/platforms`,
        'ja': `${SITE_URL}/ja/platforms`,
        'de': `${SITE_URL}/de/platforms`,
        'x-default': `${SITE_URL}/platforms`,
      },
    },
  };
}
...
export default async function platformsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const allContent = await getAllContent('platforms', validLocale);
  const articles = allContent.map(item => ({ slug: item.slug, metadata: item.metadata }));

  return <CategoryPage catKey="platforms" articles={articles} />;
}
