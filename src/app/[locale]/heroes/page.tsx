...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `${SITE_URL}/heroes`,
        'ko': `${SITE_URL}/ko/heroes`,
        'ja': `${SITE_URL}/ja/heroes`,
        'de': `${SITE_URL}/de/heroes`,
        'x-default': `${SITE_URL}/heroes`,
      },
    },
  };
}
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const allContent = await getAllContent('heroes', validLocale);
  const articles = allContent.map(item => ({ slug: item.slug, metadata: item.metadata }));

  return <CategoryPage catKey="heroes" articles={articles} />;
}
