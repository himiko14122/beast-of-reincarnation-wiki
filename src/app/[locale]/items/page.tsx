...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `${SITE_URL}/items`,
        'ko': `${SITE_URL}/ko/items`,
        'ja': `${SITE_URL}/ja/items`,
        'de': `${SITE_URL}/de/items`,
        'x-default': `${SITE_URL}/items`,
      },
    },
  };
}
...
export default async function itemsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const allContent = await getAllContent('items', validLocale);
  const articles = allContent.map(item => ({ slug: item.slug, metadata: item.metadata }));

  return <CategoryPage catKey="items" articles={articles} />;
}
