...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `${SITE_URL}/builds`,
        'ko': `${SITE_URL}/ko/builds`,
        'ja': `${SITE_URL}/ja/builds`,
        'de': `${SITE_URL}/de/builds`,
        'x-default': `${SITE_URL}/builds`,
      },
    },
  };
}
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const allContent = await getAllContent('builds', validLocale);
  const articles = allContent.map(item => ({ slug: item.slug, metadata: item.metadata }));

  return <CategoryPage catKey="builds" articles={articles} />;
}
