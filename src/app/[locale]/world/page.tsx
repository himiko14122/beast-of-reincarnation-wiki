...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `${SITE_URL}/world`,
        'ko': `${SITE_URL}/ko/world`,
        'ja': `${SITE_URL}/ja/world`,
        'de': `${SITE_URL}/de/world`,
        'x-default': `${SITE_URL}/world`,
      },
    },
  };
}
...
export default async function worldPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const allContent = await getAllContent('world', validLocale);
  const articles = allContent.map(item => ({ slug: item.slug, metadata: item.metadata }));

  return <CategoryPage catKey="world" articles={articles} />;
}
